...
def _bundled_easyocr_dir() -> Optional[str]:
    """Pakete gömülü EasyOCR model dizinini (craft + latin_g2 .pth) döndürür.

    Frozen (.exe) ortamda PyInstaller'ın _MEIPASS/easyocr_models, geliştirmede
    proje kökündeki easyocr_models klasörü. İçinde .pth varsa o yol döner; yoksa
    None (EasyOCR varsayılan ~/.EasyOCR davranışına / internet indirmesine düşer).
    Bu, internetsiz hedef makinede OCR'ın indirmeye ihtiyaç duymamasını sağlar.
    """
    base = getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(__file__)))
    cand = os.path.join(base, "easyocr_models")
    try:
        if os.path.isdir(cand) and any(f.endswith(".pth") for f in os.listdir(cand)):
            return cand
    except OSError:
        pass
    return None
...
class DocumentProcessor:
    ...
    def _get_ocr_reader(self):
        ...
        try:
            ...
            reader_kwargs = {"gpu": use_gpu}
            # Pakete gömülü modeller varsa onları kullan ve indirmeyi kapat
            # (internetsiz hedef makinede şart). Yoksa EasyOCR varsayılan
            # davranışına (~/.EasyOCR, gerekirse indirme) bırakılır.
            store = _bundled_easyocr_dir()
            if store:
                reader_kwargs["model_storage_directory"] = store
                reader_kwargs["download_enabled"] = False
                logger.info("EasyOCR gömülü model dizini: %s", store)
            self._ocr_reader = easyocr.Reader(self.ocr_languages, **reader_kwargs)
